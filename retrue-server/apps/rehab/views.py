...
class RehabPlanDetailView(APIView):
    ...
    def get(self, request, plan_id: int):
        ...
        if plan is None:
            return ApiResponse.error("康复周期不存在或无权访问", 404)
        return ApiResponse.ok(RehabPlanSerializer(plan).data, message="查询康复周期成功")
    ...
    def put(self, request, plan_id: int):
        ...
        try:
            ...
            return ApiResponse.error(str(exc), 400)
        return ApiResponse.ok(RehabPlanSerializer(updated).data, message="康复周期已更新")
...
class RehabPlanCourseListView(APIView):
    ...
    def post(self, request):
        ...
        if plan.therapist_id != request.user.id:
            return ApiResponse.error("无权操作该康复周期", 403)
        if plan.status != "active":
            return ApiResponse.error("只能为进行中的康复周期添加课程", 400)
        ...
        if data.get("status") == "completed":
            return ApiResponse.error("新建周期课程不能直接设为已完成", 400)
        if package is not None and (
            package.therapist_id != request.user.id or package.customer_id != plan.customer_id
        ):
            return ApiResponse.error("课时包与康复周期客户不一致", 400)
...
class RehabPlanCourseDetailView(APIView):
    ...
    def get(self, request, course_id: int):
        ...
        if plan_course is None:
            return ApiResponse.error("周期课程不存在或无权访问", 404)
        return ApiResponse.ok(RehabPlanCourseSerializer(plan_course).data, message="查询周期课程成功")
    ...
    def put(self, request, course_id: int):
        ...
        if plan_course.rehab_plan.status != "active":
            return ApiResponse.error("已结束的康复周期不能修改课程", 400)
        ...
        if changes_identity:
            return ApiResponse.error("所属周期和课程模板创建后不能更换；请新增周期课程", 400)
        ...
        plan_course.save()
        return ApiResponse.ok(RehabPlanCourseSerializer(plan_course).data, message="周期课程已更新")
...
class RehabPlanCourseAdjustView(APIView):
    ...
    def post(self, request, course_id: int):
        ...
        if plan_course is None:
            return ApiResponse.error("周期课程不存在或无权访问", 404)
        if plan_course.rehab_plan.status != "active":
            return ApiResponse.error("已结束的康复周期不能调整课程次数", 400)
        if plan_course.status == "cancelled":
            return ApiResponse.error("已取消的周期课程不能调整次数；请先恢复课程状态", 400)
...
class RehabStageView(APIView):
    ...
    def post(self, request):
        ...
        if plan is None:
            return ApiResponse.error("请先为客户创建进行中的康复周期", 400)
        data = dict(serializer.validated_data)
        data.pop("customer", None)
        data["plan"] = plan
        try:
