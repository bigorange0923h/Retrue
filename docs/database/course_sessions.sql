...
-- 排期是日历中的实际预约，关联客户疗程；完成后按疗程规则扣减课时。
-- ============================================================
...
CREATE TABLE tb_course_sessions (
    ...
    customer_course_id BIGINT NULL REFERENCES tb_customer_courses(id) ON DELETE SET NULL,
...
COMMENT ON COLUMN tb_course_sessions.customer_course_id IS '关联客户疗程 ID，可空';
