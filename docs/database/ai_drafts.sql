...
CREATE INDEX idx_ai_confirmation_key ON tb_ai_drafts (confirmation_key);
...
COMMENT ON COLUMN tb_ai_drafts.assistant_task_id IS '统一助手任务 ID，可空；由服务层校验任务归属和类型';
COMMENT ON COLUMN tb_ai_drafts.training_record_id IS '确认后关联的正式训练记录 ID，可空';
...
COMMENT ON COLUMN tb_ai_drafts.confirmation_key IS '确认请求幂等键；不声明数据库物理外键';
