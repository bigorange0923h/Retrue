...
CREATE TABLE tb_course_types (
    ...
    default_stage VARCHAR(12) NOT NULL DEFAULT '',
...
COMMENT ON TABLE tb_course_types IS '课程类型表，康复师维护的课程目录定义';
...
COMMENT ON COLUMN tb_course_types.is_active IS '启用状态；被疗程引用的类型只能停用不能删除';
...
COMMENT ON COLUMN tb_course_types.default_stage IS '默认适用康复阶段';
COMMENT ON COLUMN tb_course_types.default_goals IS '默认课程目标';
COMMENT ON COLUMN tb_course_types.default_notes IS '默认注意事项';
