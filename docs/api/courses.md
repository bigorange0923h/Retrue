...
只能为当前康复师名下客户排课。`plan_course` 可空；选择时必须属于该客户进行中的康复周期课程，已结束周期不能继续排课。首次评估等计划外课程可以不关联周期课程。开始/结束时间必须同时填写或同时留空，且结束时间必须晚于开始时间；新建时不能直接传 `completed`。前端选择周期课程后会按其时长快照自动计算结束时间。

## 管理单节课程

`GET /api/courses/{id}/` 查询课程详情；`PUT /api/courses/{id}/` 可更新客户、周期课程、课时、日期、时间、备注及状态。
...
- `PUT /api/courses/course-types/{id}/`：更新或停用模板。停用不影响历史周期课程。

周期课程的创建、更新和次数调整接口见 `rehab.md`。
